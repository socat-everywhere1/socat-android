...
data class Profile(
        ...
        var remotePort: Int = 5400,
        var password: String = "pwd",
        var method: String = "aes-128-cfb",

        var route: String = Acl.BYPASS_LAN,
        var remoteDns: String = "1.1.1.1",
        var proxyApps: Boolean = false,
        var bypass: Boolean = false,
        var udpdns: Boolean = false,
        var ipv6: Boolean = true,
        var localIP: String = "10.2.1.2",
        var peerID: String = "tun1P1",
        var socatPort: Int = 10211,
        var sourcePort: Int = 10212,
        ...
        private class JsonParser(private val feature: Profile? = null) : ArrayList<Profile>() {
            ...
            private fun tryParse(json: JsonObject, fallback: Boolean = false): Profile? {
                ...
                return Profile().also {
                    ...
                    localIP = json["local_ip"].optString?: localIP
                    peerID =  json["peer_id"].optString?:  peerID
                    socatPort = json["socat_port"].optInt?: socatPort
                    sourcePort = json["source_port"].optInt?: sourcePort
                    isSponsored = json["is_sponsored"].optBoolean ?: isSponsored
    ...
    fun toJson(profiles: LongSparseArray<Profile>? = null): JSONObject = JSONObject().apply {
        ...
        put("socat_port", socatPort)
        put("source_port", sourcePort)
        put("peer_id", peerID)
        put("is_sponsored",isSponsored)
        put("ipv6", ipv6)
        put("metered", metered)
    ...
    fun serialize() {
        ...
        DataStore.privateStore.putString(Key.localIP, localIP)
        DataStore.privateStore.putString(Key.socatPort, socatPort.toString())
        DataStore.privateStore.putString(Key.sourcePort, sourcePort.toString())
        DataStore.privateStore.putString(Key.peerID, peerID)
        DataStore.privateStore.putBoolean(Key.isSponsored, isSponsored)
    ...
    fun deserialize() {
        ...
        localIP = DataStore.privateStore.getString(Key.localIP) ?: ""
        socatPort = parsePort(DataStore.privateStore.getString(Key.socatPort), 1, 1)
        sourcePort = parsePort(DataStore.privateStore.getString(Key.sourcePort), 1, 1)
        peerID = DataStore.privateStore.getString(Key.peerID) ?: ""
        isSponsored = DataStore.privateStore.getBoolean(Key.isSponsored, false)
